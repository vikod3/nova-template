...
const Navbar = () => {
  ...
  return <nav className="absolute top-4 left-4 right-4 z-50">
      <div className="w-full px-4 py-3 rounded-2xl border border-white/12 backdrop-blur-[20px] flex justify-between items-center" style={{
      background: 'rgba(21, 22, 26, 0.80)'
    }}>
        {/* Logo and Brand */}
        <div className="flex items-center gap-7">
          <div className="flex items-center gap-3">
            <h1 className="text-white text-lg font-semibold leading-7 font-sans">Floney</h1>
          </div>
          ...
          <div className="hidden lg:flex flex-1 justify-between items-center gap-8">
            {navItems.map(item => <button key={item.name} className="px-2.5 py-2 text-sm leading-5 font-sans text-white transition-opacity hover:opacity-80 font-normal">
                {item.name}
              </button>)}
          </div>
        </div>
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            <SheetContent side="right" className="w-[300px] bg-[#15161a]/95 border-white/12 backdrop-blur-[20px]">
              <div className="flex flex-col space-y-6 mt-8">
                {navItems.map(item => <button key={item.name} className="text-left px-4 py-3 text-base text-white font-semibold transition-opacity hover:opacity-80" onClick={() => setIsOpen(false)}>
                    {item.name}
                  </button>)}
